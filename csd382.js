...
class Network {
    constructor(inputs, layerSetupArray) {
        ...
        this.momentum = .002509
        this.learningRate = .002509
...
//     // ing.src = `r${t}.png`
//     guys.push(ing)
//     let pix1 = canvas_context.getImageData(0, 0, 32, 32)
//     let pix1z = canvas_context.getImageData(0, 0, 32, 32)
//     let pix1z2 = canvas_context.getImageData(0, 0, 32, 32)
//     pixboxes.push(pix1)
...
// let bw = new Image()
let b = new Rectangle(0, 0, 100, 50, "black")
let w = new Rectangle(0, 50, 100, 50, "black")
let c = new Rectangle(25, 25, 50, 50, "white")
// b.draw()
// w.draw()
// c.draw()
let pix2 = canvas_context.getImageData(100, 100, 32, 32)
let pix3 = canvas_context.getImageData(100, 100, 32, 32)

// b.draw()
// w.draw()
// c.draw()
pix1 = canvas_context.getImageData(0, 0, 32, 32)

let inputs = []
for (let t = 0; t < pix1.data.length; t++) {
    if (t % 4 == 3) {
    } else {
...
let net = new Network(inputs, [256, 256, inputs.length])
...
function exportJSON(originalData) {
    ...
    a.setAttribute("download", `256FruitNet.js`);
...
function main() {
    ...
    if (start >= 1) {
        if (keysPressed['n']) {
            ...
            // canvas_context.putImageData(pix3, 0, 0)
            statpixboxes2[count] = canvas_context.getImageData(0, 0, 32, 32)

        } else {
            // statpixboxes[count] = canvas_context.getImageData(0, 0, 32, 32)
        }
        ...
        // canvas_context.putImageData(statpixboxes[count], 64, 32)
        ...
        canvas_context.putImageData(pix3, (count % 13) * 32, Math.floor(count / 13) * 32)
        // statpixboxes[count] = canvas_context.getImageData(0, 0, 32, 32)

        // canvas_context.fillStyle = "#888888"
        // canvas_context.fillRect

        // canvas_context.putImageData(statpixboxes[count], 0, 32)
        // canvas_context.fillRect(32, 0, 32, 32)
        // // canvas_context.drawImage(guys[count], 0, 0, 64, 64, 32, 0, 32, 32)

        // canvas_context.drawImage(frui, (count%(91))*270, 0, 270,270, 32, 0, 32, 32) 
        // canvas_context.putImageData(statpixboxes2[count], 32, 32)

        // statpixboxes[count] = canvas_context.getImageData(0, 0, 32, 32)
        // canvas_context.putImageData(statpixboxes[count], 64, 0)
